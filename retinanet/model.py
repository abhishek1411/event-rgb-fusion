...
class ResNet_early_fusion(nn.Module):
    ...
    def forward(self, inputs):
        ...
        input_batch = torch.cat((img_batch_event,img_batch_rgb),1)
...
def resnet50(num_classes, fusion_model, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    if fusion_model == 'early_fusion':
...
def resnet101(num_classes,fusion_model, pretrained=False, **kwargs):
    ...
    """
    if fusion_model == 'early_fusion':
        model = ResNet_early_fusion(num_classes, Bottleneck, [3, 4, 23, 3], **kwargs)
    if fusion_model == 'fpn_fusion':
        model = ResNet_fpn_fusion(num_classes, Bottleneck, [3, 4, 23, 3], **kwargs)
    if fusion_model == 'event':
        model = ResNet(num_classes, Bottleneck, [3, 4, 23, 3], **kwargs)
    if fusion_model == 'rgb':
        model = ResNet_rgb(num_classes, Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
