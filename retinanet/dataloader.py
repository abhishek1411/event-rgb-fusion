...
class CSVDataset_gray(Dataset):
    ...
    def load_image(self, image_index):
        ...
        event_file = os.path.join(self.event_dir, self.image_names[image_index])
        event_file = event_file.replace('.npz','.png')
        img = cv2.imread(event_file, cv2.IMREAD_GRAYSCALE)
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=480, max_side=640):
        ...
        return {'img': sample['img'], 'img_rgb': torch.from_numpy(image), 'annot': torch.from_numpy(annots), 'scale': scale}
